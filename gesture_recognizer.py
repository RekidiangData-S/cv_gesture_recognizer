            ...
            if self.bg is None:
                ...
                try:
                    ...
                    hand_crop_resized = np.expand_dims(cv2.resize(
                        hand_crop, (54, 54)), axis=0).reshape((1, 54, 54, 1))
                    ...
                    for i, pred in enumerate(prediction[0]):
                        ...
                        bary = 60 + i*60
                        bar_height = 20
                        # calculate length of confidence bar
                ...
                mouse_change = ((p1[0] + p2[0])//2 - self.positions['null_pos']
                                [0], self.positions['null_pos'][0] - (p1[1] + p2[1])//2)

                # Draw hand moved difference
                cv2.circle(
                    display, self.positions['null_pos'], 5, (0, 0, 255), -1)
                cv2.circle(display, hand_pos, 5, (0, 255, 0), -1)
                cv2.line(
                    display, self.positions['null_pos'], hand_pos, (255, 0, 0), 5)
                ...
                cv2.putText(foreground_display, "FPS : " + str(int(fps)),
                            self.positions['fps'], cv2.FONT_HERSHEY_SIMPLEX, 0.65, (50, 170, 50), 2)
